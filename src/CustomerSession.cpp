...
void CustomerSession ::prepareBill() {
  ...
  // pay it normally, create a tab, add to existing tab, or pay bill and tab
  // total
  ...
  cout << "Your bill total is R" << std::setprecision(2) << fixed
       << tableOrderBuilder->getOrder()->getCost() << endl;
  cout << endl;
  cout << "You have 5 options on how to handle your bill." << endl;
  cout << "Enter the number corresponding to one of the following options, "
          "then press ENTER:"
       << endl;
  cout << endl;
  ...
  if (input == 1) {
    ...
    cout << "Could you please provide us with your first name below, then "
            "press ENTER:";
    string nameInput;
    ...
    if (!tabExistence(nameInput)) {
      ...
      cout << "Your tab has been successfully created. Please note that you "
              "have a maximum of R3000, you will then have to pay."
           << endl;
      cout << "Tab created successfully. Thank you for choosing WackDonalds, "
              "see you soon!"
           << endl;
      return;
    ...
    if (!tabExistence(nameInput)) {
      string errorMessage3 =
          "You do not have a tab open with us. Please try again.";
      ...
      if (currCustomerTab->getAmount() >= 3000) {
        cout << "Unfortunately you have reached the tab limit of over "
                "R3000.00. Please pay everything immediately";

        tableBill = new SingleBill(tableOrderBuilder->getOrder()->getCost() +
                                   currCustomerTab->getAmount());
        // payBill(tableOrderBuilder->getOrder()->getCost() +
        // currCustomerTab->getAmount());
...
void CustomerSession ::payBill(double billAmount) {
  ...
  if (SplitBill *split = dynamic_cast<SplitBill *>(tableBill)) {
    ...
    for (Bill *ptr : split->getBills()) {
      ...
      cout << "Payment number " + to_string(i) + " of the amount R"
           << std::setprecision(2) << fixed
           << tableOrderBuilder->getOrder()->getCost() << " is successful."
           << endl;
      i++;
    }
    cout << "Payments all successful. Thank you for choosing WackDonalds, see "
            "you soon!"
         << endl;
    ...
    cout << "\nCustomer is currently paying this single bill of R"
         << std::setprecision(2) << fixed << billAmount << "..." << endl;

    cout << "Payment successful. Thank you for choosing WackDonalds, see you "
            "soon!"
         << endl;
